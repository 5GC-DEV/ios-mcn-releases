...
# **IOS MCN v0.5.0 Agartala Release: CORE Developer Guide**
...
```sh
wget https://github.com/ios-mcn/ios-mcn-releases/raw/refs/heads/main/Agartala/v0.5.0/CORE/source-code/iosmcn.agartala.v0.5.0.core.source.tar.gz
```

Step 3: Untar the iosmcn.agartala.v0.5.0.core.source.tar.gz.

```sh
tar -xvzf iosmcn.agartala.v0.5.0.core.source.tar.gz
cd iosmcn.agartala.v0.5.0.core.source
```

Step 4: Untar the network functions -  *\<nf>*-0.0.*\<nf-version>*.iosmcn.core.*\<nf>*.tar.gz.

```sh
tar -xvzf amf-0.5.0.iosmcn.core.amf.tar.gz
cd amf-0.5.0.iosmcn.core.amf
```
or

```sh
tar -xvzf smf-0.5.0.iosmcn.core.smf.tar.gz
cd smf-0.5.0.iosmcn.core.smf
```
...
...
- Update the image name to the newly built image under the _tags_ section of the yaml file:
	...
	5g-control-plane:
	  ...
	  images:
		...
		tags:
		  amf: docker.io/iosmcncorereg/5gc-amf:release-0.3.2
		  smf: docker.io/iosmcncorereg/5gc-smf:release-0.3.2
		  pcf: docker.io/iosmcncorereg/5gc-pcf:release-0.3.1
		  nrf: docker.io/iosmcncorereg/5gc-nrf:release-0.2.1
		  ausf: docker.io/iosmcncorereg/5gc-ausf:release-0.2.1
		  nssf: docker.io/iosmcncorereg/5gc-nssf:release-0.3.1
		  udm: docker.io/iosmcncorereg/5gc-udm:release-0.3.2
		  udr: docker.io/iosmcncorereg/5gc-udr:release-0.3.2
		  webui: docker.io/iosmcncorereg/5gc-webui:release-0.3.1
	...
	omec-user-plane:
	  ...
	  images:
	    ...
	    tags:
	      bess: docker.io/iosmcncorereg/upf-epc-bess:release-ivybridge-0.3.2
	      pfcpiface: docker.io/iosmcncorereg/upf-epc-pfcpiface:release-ivybridge-0.3.2
	```

- If the core is installed and you want to update the pod with the new images and reset the core, run the following command:
	```sh
	cd ~/IOSMCN-Core/iosmcn.agartala.v0.5.0.core.images/IOSMCN-CoreDpm
